...
namespace ICSharpCode.Core.Presentation
{
	...
	public class InputGestureCollectionConverter : TypeConverter
	{
		...
		/// <summary>
		/// Parses the shortcut text. If there are warnings (e.g. SD3 syntax for shortcuts), context is displayed on the console.
		/// </summary>
		public static InputGestureCollection Parse(string shortcut, object context)
		{
			if (shortcut == null)
				throw new ArgumentNullException("shortcut");
			if (shortcut.Contains("|")) {
				LoggingService.Warn("Old-syntax shortcut in " + context);
				shortcut = shortcut.Replace('|', '+');
			}
			
			var gestures = new InputGestureCollection();
			var serializedGestures = Regex.Split(shortcut, @"\s*\;\s*");
			...
			foreach (var serializedGesture in serializedGestures) {
				...
				if (serializedGesture.IndexOf(",") < 0) {
					var keyGesture = (InputGesture)new KeyGestureConverter().ConvertFromInvariantString(serializedGesture);
					gestures.Add(keyGesture);
				} else {
					var multiKeyGestyre = (InputGesture)new MultiKeyGestureConverter().ConvertFromInvariantString(serializedGesture);
		...
		public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value) {
			if (value is string) {
				return Parse((string)value, context);
			}
