...
namespace ICSharpCode.WpfDesign.Tests.Designer
{
	...
	public class ModelTests : ModelTestHelper
	{
		...
		{
			DesignItem button = CreateCanvasContext("<Button/>");
			var s = button.Context.UndoService;
		...
		public void UndoRedoChangeGroupTest()
		{
			...
			Assert.IsFalse(s.CanUndo);
			Assert.IsFalse(s.CanRedo);
			using (var g = button.OpenGroup("Resize")) {
				button.Properties["Width"].SetValue(100.0);
		...
		public void AddTextBoxToCanvas()
		{
			...
			DesignItem textBox = canvas.Context.ModelService.CreateItem(new TextBox());
		...
		public void InsertTextBoxInCanvas()
		{
			...
			DesignItem checkBox = canvas.Context.ModelService.CreateItem(new CheckBox());
		...
		public void ClearImplicitChildCollection()
		{
			...
			AssertCanvasDesignerOutput("<Canvas/>", canvas.Context);
			AssertLog("");
		}
	}
}
