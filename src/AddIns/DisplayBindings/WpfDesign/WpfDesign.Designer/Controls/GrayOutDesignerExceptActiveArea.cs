...
namespace ICSharpCode.WpfDesign.Designer.Controls
{
	...
	sealed class GrayOutDesignerExceptActiveArea : FrameworkElement
	{
		...
		internal static void Start(ref GrayOutDesignerExceptActiveArea grayOut, DesignContext context, UIElement activeContainer)
		{
			Debug.Assert(activeContainer != null);
			Start(ref grayOut, context, activeContainer, new Rect(activeContainer.RenderSize));
		}

		internal static void Start(ref GrayOutDesignerExceptActiveArea grayOut, DesignContext context, UIElement activeContainer, Rect activeRectInActiveContainer)
		{
			Debug.Assert(context != null);
			Debug.Assert(activeContainer != null);
			DesignPanel designPanel = context.DesignPanel as DesignPanel;
			OptionService optionService = context.GetService<OptionService>();
			if (designPanel != null && grayOut == null && optionService != null && optionService.GrayOutDesignSurfaceExceptParentContainerWhenDragging) {
				...
				grayOut.adornerPanel.SetAdornedElement(designPanel.Context.ModelService.Root.View, null);
