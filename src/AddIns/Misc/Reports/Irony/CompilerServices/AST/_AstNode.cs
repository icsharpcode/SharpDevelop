...
// statements that would be otherwise needed.
namespace Irony.Compiler {
  ...
  public class AstNode {
    ...
    public AstNode(NodeArgs args){
      if (args == null) return; //to allow construction of nodes outside the parser context
      this.Evaluate = DoEvaluate; //default implementation 
      Term = args.Term;
      Span = args.Span;
      ...
      foreach (AstNode child in args.ChildNodes)
        AddChild(null, child);
      //inherit precedence from child node.
      if (Precedence == NoPrecedence && ChildNodes.Count == 1)
        Precedence = ChildNodes[0].Precedence;
    ...
    public string GetContent() {
      if (this is Token)
        return (this as Token).ValueString;
      //Quick path
      if (ChildNodes.Count > 0) {
        Token tkn = ChildNodes[0] as Token;
